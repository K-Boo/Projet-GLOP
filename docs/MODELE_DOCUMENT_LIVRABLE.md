# MODÈLE OFFICIEL DE PRÉSENTATION DES DOCUMENTS & LIVRABLES (GLOP)
## Université de Lille — Faculté des Sciences et Technologies — M2 MIAGE

---

<!-- EN-TÊTE GRAPHIQUE AVEC LES DEUX LOGOS OFFICIELS -->
<div align="center" style="margin-bottom: 25px;">
  <table style="width: 100%; border: none; background: transparent;">
    <tr style="border: none; background: transparent;">
      <td align="left" style="width: 50%; border: none; vertical-align: middle;">
        <img src="assets/logo_univ_lille.png" alt="Logo Université de Lille" style="height: 60px; max-width: 250px; object-fit: contain;">
      </td>
      <td align="right" style="width: 50%; border: none; vertical-align: middle;">
        <img src="assets/logo_fst_informatique.png" alt="Logo FST Département Informatique" style="height: 60px; max-width: 250px; object-fit: contain;">
      </td>
    </tr>
  </table>
</div>

---

<!-- CARTOUCHE D'INFORMATION OFFICIELLE SUR LE DOCUMENT -->
## 📋 Informations Générales sur le Document

| Champ | Information |
|---|---|
| **Intitulé du Projet** | **Projet ShopLoc** — Marketplace & Fidélisation multi-commerces |
| **Identifiant Officiel du Projet** | `MiageShopLoc` |
| **Titre du Document** | **[Insérer le titre exact du document]** (ex : *Cahier des Charges & Réponse à l'Appel d'Offres*) |
| **Référence Documentaire** | `GLOP-2026-R1-CDC-v1.0` *(Nomenclature : UE-ANNÉE-LIVRABLE-TYPE-VERSION)* |
| **Contexte Académique** | Master 2 MIAGE — UE Génie Logiciel par la Pratique (GLOP) 2026-2027 |
| **Maîtrise d'Ouvrage (Clients)** | Laurence Duchien, Anne Etien, François Secchi, Jérémy Woirhaye |
| **Date de Rédaction / Soumission** | 07 Septembre 2026 |
| **Statut du Document** | Version 1.0 — *Pour validation formelle* |
| **Nombre Total de Pages** | [X] pages *(Sommaire requis si > 10 pages)* |
| **Tag obligatoire communications** | `[GLOP]` *(à inclure dans tout objet de courriel)* |

---

## 📜 Historique des Versions du Document

| Version | Date | Auteur(s) | Nature des Modifications apportées |
|---|---|---|---|
| `v0.1` | 03/09/2026 | Équipe Projet | Ébauche initiale et structure du document. |
| `v1.0` | 07/09/2026 | Équipe Projet | Version complète soumise pour instruction et arbitrage de la MOA. |

---

<!-- SOMMAIRE AUTOMATIQUE OBLIGATOIRE SI LE DOCUMENT FAIT PLUS DE 10 PAGES -->
## 📑 Table des Matières
*(À générer obligatoirement dès que le document dépasse 10 pages conformément à la consigne de la slide 31)*

1. [Contexte et Objectifs](#)
2. [Périmètre Fonctionnel & Personas](#)
3. [Spécifications Métiers Détaillées](#)
4. [Architecture Technique & Contraintes](#)
5. [Planning & Estimation Budgétaire](#)
6. [Glossaire et Références](#)

---

<!-- CORPS DU DOCUMENT -->
# 1. [Titre de la Première Section]

Contenu rédigé du livrable...
